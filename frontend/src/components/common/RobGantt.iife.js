    ...
    get_snap_position(dx) {
      ...
      if (this.gantt.view_is('Week')) {
        ...
        position =
          odx -
          rem +
          (rem < this.gantt.options.column_width / 14 ? 0 : this.gantt.options.column_width / 7)
      } else if (this.gantt.view_is('Month')) {
        rem = dx % (this.gantt.options.column_width / 30)
        position =
          odx -
          rem +
          (rem < this.gantt.options.column_width / 60 ? 0 : this.gantt.options.column_width / 30)
      } else {
        rem = dx % this.gantt.options.column_width
        position =
          odx -
          rem +
          (rem < this.gantt.options.column_width / 2 ? 0 : this.gantt.options.column_width)
      }
      return position
    }
  ...
  class Gantt {
    ...
    update_view_scale(view_mode) {
      ...
      if (view_mode === VIEW_MODE.HOUR) {
        ...
        this.options.column_width = 120
      } else if (view_mode === VIEW_MODE.YEAR || view_mode === VIEW_MODE.FISCAL_YEAR) {
    ...
    setup_gantt_dates() {
      ...
      } else if (this.view_is([VIEW_MODE.YEAR, VIEW_MODE.FISCAL_YEAR])) {
        this.gantt_start = date_utils.add(this.gantt_start, -2, 'year')
        this.gantt_end = date_utils.add(this.gantt_end, 2, 'year')
      } else if (this.view_is(VIEW_MODE.QUARTER)) {
        this.gantt_start = date_utils.start_of(this.gantt_start, -2, 'year')
        this.gantt_end = date_utils.add(this.gantt_end, 2, 'year')
      } else {
    ...
    setup_date_values() {
      ...
      while (cur_date === null || cur_date < this.gantt_end) {
        if (!cur_date) {
          ...
          if (this.view_is(VIEW_MODE.YEAR) || this.view_is(VIEW_MODE.FISCAL_YEAR)) {
    ...
    get_date_info(date, last_date, i) {
      ...
      const date_text = {
        ...
        'Fiscal Year_lower': format(date, 'yyyy'),
        'Quarter Day_upper':
          date.getDate() !== last_date.getDate()
            ? date_utils.format(date, 'D MMM', this.options.language)
            : '',
      ...
      const x_pos = {
        ...
        'Fiscal Year_lower': this.options.column_width / 2,
        'Fiscal Year_upper': (this.options.column_width * 30) / 2
      }
    ...
    get_snap_position(dx) {
      ...
      if (this.view_is(VIEW_MODE.WEEK)) {
        ...
        position =
          odx - rem + (rem < this.options.column_width / 14 ? 0 : this.options.column_width / 7)
      } else if (this.view_is(VIEW_MODE.MONTH)) {
        rem = dx % (this.options.column_width / 30)
        position =
          odx - rem + (rem < this.options.column_width / 60 ? 0 : this.options.column_width / 30)
      } else if (this.view_is(VIEW_MODE.QUARTER)) {
        rem = dx % (this.options.column_width / 120)
        position =
          odx - rem + (rem < this.options.column_width / 240 ? 0 : this.options.column_width / 120)
      } else if (this.view_is(VIEW_MODE.YEAR) || this.view_is(VIEW_MODE.FISCAL_YEAR)) {
        rem = dx % (this.options.column_width / 365)
        position =
          odx - rem + (rem < this.options.column_width / 730 ? 0 : this.options.column_width / 365)
      } else {
