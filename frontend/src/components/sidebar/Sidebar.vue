<template>
  <div class="hidden lg:fixed lg:inset-y-0 lg:z-50 lg:flex lg:w-72 lg:flex-col">
    ...
    <div class="flex grow flex-col gap-y-5 overflow-y-auto border-r border-gray-200 bg-white px-6">
      <div class="flex h-8 my-8 shrink-0 items-center">
        ...
        <ul role="list" class="flex flex-1 flex-col gap-y-7">
          <li>
            <ul role="list" class="-mx-2 space-y-1">
              <li v-for="(menu, key) in menus" :key="key">
                <router-link :to="menu.link" class="text-gray-700 hover:text-slate-600 hover:bg-gray-50 group flex gap-x-3 rounded-md p-2 text-sm leading-6 font-semibold">
                  <component :is="menu.icon" class="text-gray-400 group-hover:text-gray-500 flex-shrink-0 h-6 w-6" aria-hidden="true"></component>
                  {{ menu.title }}
                </router-link>
              </li>
            </ul>
          ...
          <li class="-mx-6 mt-auto">
            <a href="#" class="flex items-center gap-x-4 px-6 py-3 text-sm font-semibold leading-6 text-gray-900 hover:bg-gray-50">
              ...
              <span aria-hidden="true">Rob Hang</span>
            </a>
          </li>
        </ul>
      </nav>
...
import { FolderPlusIcon, WrenchScrewdriverIcon, ChatBubbleLeftEllipsisIcon, UserGroupIcon } from '@heroicons/vue/24/solid'
